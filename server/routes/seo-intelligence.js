/**
 * SEO Intelligence route – port of supabase/functions/seo-intelligence/index.ts
 * to Node.js / Express.  Uses only native fetch (Node 18+).
 */
import { Router } from 'express';

export const seoIntelligenceRouter = Router();

// ----------- helpers (same logic as original Deno function) -----------
...
function detectCanonicalConflict(html, pageUrl) {
  const canonical = extractCanonical(html);
  if (!canonical) return false;
  try {
    return new URL(canonical).href !== new URL(pageUrl).href;
  } catch {
    return false;
  }
}
...
function countMissingAltTags(html) {
  let missing = 0;
...
function extractInternalUrls(html, baseUrl) {
  const urls = [];
  try {
...
function calculateKeywordDensityPercentage(text) {
...
function extractTopAnchors(html) {
  const anchors = {};
...
function calculateContentDepth(wordCount, headings) {
  let s = 0;
  if (wordCount > 300) s += 2;
  if (wordCount > 1000) s += 2;
  if (wordCount > 2000) s += 1;
  if (headings.h2.length > 2) s += 1;
  if (headings.h3.length > 3) s += 1;
  if (headings.h2.length > 5) s += 1;
  return Math.min(s, 10);
}

function calculateContentUniqueness(text) {
  const words = text.toLowerCase().split(/\s+/);
  return Math.round((new Set(words).size / words.length) * 100);
}

function calculateOrphanRisk(internalLinkCount, pageDepth) {
  let risk = 0;
...
async function checkSitemapXml(baseUrl) {
  try {
    const u = new URL(baseUrl);
...
async function detectRedirectChain(url) {
  const chain = [url];
  let current = url, max = 5;
  try {
    while (max-- > 0) {
      const res = await fetch(current, { redirect: 'manual' });
      if (res.status >= 300 && res.status < 400) {
        const loc = res.headers.get('location');
        if (loc) { current = new URL(loc, current).href; chain.push(current); }
        else break;
      } else break;
    }
  } catch { /* ignore */ }
  ...
  try {
    ...
    for (const m of html.matchAll(/<a[^>]*href=["']([^"']*)["']/gi)) {
      ...
      try {
        if (href.startsWith('http://') || href.startsWith('https://')) {
          fullUrl = href;
          isInternal = new URL(href).hostname === baseDomain;
        } else {
...
function generateRecommendations(a) {
  ...
  if (a.technical_seo.canonical_conflict) recs.push('WARNING: Canonical URL conflicts with page URL - check for redirect issues');
  ...
  if (a.technical_seo.noindex) recs.push('WARNING: Page has noindex meta tag - it will not be indexed by search engines');
  if (a.technical_seo.missing_alt_tags > 0) recs.push(`Add ALT tags to ${a.technical_seo.missing_alt_tags} images for accessibility and SEO`);
  if (a.technical_seo.broken_internal_links > 0) recs.push(`Fix ${a.technical_seo.broken_internal_links} broken internal links detected`);
  if (a.technical_seo.broken_external_links > 0) recs.push(`Fix ${a.technical_seo.broken_external_links} broken external links detected`);
  if (!a.technical_seo.structured_data_valid && a.technical_seo.structured_data.length > 0) recs.push('Fix invalid structured data (JSON-LD) on the page');
  if (a.content_analysis.headings.h1.length > 1) recs.push('Use only one H1 heading per page for better SEO');
  if (a.content_analysis.headings.h2.length < 2 && a.meta.word_count > 300) recs.push('Add more H2 headings to structure your content better');
  if (a.content_analysis.content_uniqueness_score < 40) recs.push('Improve content uniqueness - detected high repetition');
  if (!a.performance.viewport_meta) recs.push('Add viewport meta tag for mobile-friendliness');
  if (!a.performance.font_size_appropriate) recs.push('Increase font size for better mobile readability (minimum 14px)');
  if (a.performance.estimated_lcp === 'poor') recs.push('Optimize images and reduce page size to improve LCP');
  if (a.performance.estimated_cls_risk === 'high') recs.push('Add lazy loading and image dimensions to reduce CLS risk');
  if (a.performance.estimated_inp_risk === 'high') recs.push('Reduce JavaScript execution to improve INP');
  if (a.site_structure.orphan_risk_score > 50) recs.push('WARNING: High orphan page risk - add more internal links to this page');
  if (a.site_structure.internal_link_count < 3) recs.push('Add more internal links to improve site navigation');
...
function createErrorResponse(url, status) {
  return {
    url, status,
...
seoIntelligenceRouter.post('/', async (req, res) => {
  try {
    ...
    if (!url) {
      return res.status(400).json(createErrorResponse('', 'error: URL is required'));
    }
